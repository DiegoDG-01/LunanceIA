...
class GetInstallmentPurchasesHandler:
    ...
    async def handle(self, query: GetInstallmentPurchasesQuery) -> list[InstallmentPurchaseResponseDTO]:
        user = await self.user_repository.get_by_id(query.user_id)
        if not user or not user.is_active:
            raise UserNotFoundError()

        purchases = await self.installment_purchase_repository.get_all_by_user_id(query.user_id)
        result = []
        for purchase in purchases:
            account = await self.account_repository.get_by_id(cast(int, purchase.account_id))
            charges = await self.installment_charge_repository.get_by_purchase_id(cast(int, purchase.id))
