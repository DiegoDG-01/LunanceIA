...
class AuthTokenRepository(ABC):
    ...
    @abstractmethod
    async def save_refresh_token(self, user_id: int, refresh_hash_token: str, expires_at: datetime) -> bool:
        """Save refresh token"""
        pass

    @abstractmethod
    async def get_refresh_token(self, user_id: int, refresh_hash_token: str) -> Optional[dict]:
        """Get refresh token"""
        pass
