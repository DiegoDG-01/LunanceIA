...
class PositionCapRequest(BaseModel):
    ...
    overflow_action: OverflowAction | None = Field(
        None,
        description=(
    ...
    overflow_position_uuid: str | None = Field(
...
class CreatePositionRequest(BaseModel):
    ...
    lock_period_end_date: date | None = Field(
        None, description="Fin del periodo de permanencia (no se puede liquidar antes)"
    )
    early_withdrawal_penalty: Decimal | None = Field(
        None,
        ge=Decimal(0),
        le=Decimal(100),
    ...
    cap: PositionCapRequest | None = Field(
        None,
        description=(
